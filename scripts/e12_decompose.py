"""
E12 — the three-channel decomposition on a ViT. One (arm, seed) per invocation.

Contract: docs/E12_prereg.md sec 3. Measures, per old task k at final theta_T:
...
from scripts.channel_decomp import refit_probe, LBFGS_MAX_ITER, _SKLEARN_VERSION
...
def main():
    ...
    ap.add_argument("--arm", choices=["base", "adapt", "cil"], required=True)
    ...
    ckpt_dir = os.path.join(args.ckpt_root,
                            f"ckpt_e12_{'cil' if class_il else args.arm}_seed{args.seed}",
                            f"mafc_seed{args.seed}")
    out_path = args.out or f"runs/e12_decomp/{args.arm}_{args.seed}.json"

    print("=" * 100)
    print(f"E12 DECOMPOSITION — arm={args.arm} seed={args.seed} device={device}")
    print("=" * 100)
    ...
    if class_il:
        ...
        clf100 = LogisticRegression(max_iter=LBFGS_MAX_ITER, solver="lbfgs", C=1.0)
        clf100.fit(sc100.transform(Xtr), Ytr)
        # GUARD MIRRORED FROM refit_probe -- same threshold, same message, so
        # the two paths are auditable by one grep. This probe ran 57 cells
        # without ever inspecting n_iter_; the paper could not describe those
        # fits the way it describes the others. Now it can, or it reports why not.
        n_iter_100 = int(getattr(clf100, "n_iter_", [0])[0])
        if n_iter_100 >= LBFGS_MAX_ITER:
