"""
E12 — the frozen-probe arm: the trivial use of the assumed resource, and P1's
reference.

Contract: docs/E12_prereg.md sec 1 (arms), sec 2 (P1).
...
from src.models.vit_base import ViTEncoder, VIT_MODEL
...
def main():
    ap = argparse.ArgumentParser(description="E12 frozen-probe arm")
    ap.add_argument("--root", default="./data")
    ap.add_argument("--out", default="runs/e12_frozen_probe.json")
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    args = ap.parse_args()
    device = torch.device(args.device)

    print("=" * 96)
    print("E12 FROZEN-PROBE ARM — the trivial use of ImageNet pretraining (catch 24)")
    print("=" * 96)
    print(f"  device {device} | model {VIT_MODEL}")

    encoder = ViTEncoder(model_name=VIT_MODEL, pretrained=True).to(device).eval()
    ...
    json.dump({"model": VIT_MODEL, "rows": rows, "mean_diag": mean, "min_diag": mn,
